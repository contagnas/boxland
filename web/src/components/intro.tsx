  ...
  useEffect(() => {
    if (initWebsocket) {
      ...
      socket.onerror = (event) => {
        ...
        setTimeout(() => {
          setMessages([`I AM ${name.toUpperCase()}`, `YOU ARE NOT ${name.toUpperCase()}`, `ENTER YOUR REAL NAME`, "OR AT LEAST ONE THAT IS NOT MINE"])
        }, 2000)
      }
  ...
  return (
    <>
      ...
      <div className={`h-full w-full flex flex-col justify-center items-center bg-gradient-to-b from-yellow-500 to-pink-800 text-center space-y-6 ${introShown}`}>
        ...
        <div id="nameInputContainer" className="bg-white w-96 h-64 rounded-3xl overflow-hidden flex flex-col opacity-0 transition-opacity duration-1000">
